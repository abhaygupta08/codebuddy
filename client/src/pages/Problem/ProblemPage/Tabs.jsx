...
export default function Tabs({ tabList, color, border }) {
  ...
  return (
    <div className='w-full'>
      <div className='flex justify-between w-full px-12 overflow-x-scroll text-sm text-center bg-white border shadow cursor-pointer gap-7 no-scrollbar xl:overflow-x-hidden lg:mt-0 lg:mx-0 font-roboto xs:border-b-1 md:border-1'>
        <div
          ...
          {tabList.map((el, idx) => {
            return (
              <div
                key={idx}
                data-cy='tabs'
                className={`min-w-max py-2 px-4 mx-4 select-none
                ${
                  selected === idx
